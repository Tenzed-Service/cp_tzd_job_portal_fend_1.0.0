...
import { Banners, FashionThree } from '../../../../shared/interface/theme.interface';
...
export class Fashion3Component {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<FashionThree>;
  @Select(CategoryState.category) category$: Observable<CategoryModel>;
  @Select(BrandState.brands) brand$: Observable<Select2Data>;

  public form: FormGroup
  public active = 'home_banner';
  public page_data: FashionThree;
  public banner = 1;
  ...
  ngOnInit() {
    const home_page$ = this.store.dispatch(new GetHomePage({slug: "fashion_three"}));
  ...
  removeHomeBanner(index: number) {
   ...
   // Merge Products Ids
  concatDynamicProductKeys(obj: FashionThree) {
    const result: number[]= [];
