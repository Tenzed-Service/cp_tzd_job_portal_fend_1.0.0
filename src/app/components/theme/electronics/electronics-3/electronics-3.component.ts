...
import { Banners, ElectronicThree } from '../../../../shared/interface/theme.interface';
...
@Component({
  ...
  templateUrl: './electronics-3.component.html',
  styleUrl: './electronics-3.component.scss'
})
export class Electronics3Component {

  @Select(ThemeState.homePage) home_page$: Observable<ElectronicThree>;
  ...
  public page_data: ElectronicThree;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePage({ slug: "electronics_three" }));
  ...
  patchForm() {
    this.home_page$.subscribe(homePage => {
      ...
      this.selectedCategories = homePage?.content?.category_product_1?.categories?.category_ids || [];
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: ElectronicThree) {
    const result: number[] = [];
