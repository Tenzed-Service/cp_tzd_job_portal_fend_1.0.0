...
import { Banners, FurnitureOne } from '../../../../shared/interface/theme.interface';
...
export class Furniture1Component {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<FurnitureOne>;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePage({ slug: "furniture_one" }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: FurnitureOne) {
    const result: number[] = [];
