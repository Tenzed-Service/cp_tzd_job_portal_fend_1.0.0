...
import { Banners, Christmas } from '../../../shared/interface/theme.interface';
...
export class ChristmasComponent {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<Christmas>;
  @Select(CategoryState.category) category$: Observable<CategoryModel>;
  @Select(BrandState.brands) brand$: Observable<Select2Data>;

  public form: FormGroup;
  public page_data: Christmas;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePage({ slug: "christmas" }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: Christmas) {
    const result: number[]= [];
