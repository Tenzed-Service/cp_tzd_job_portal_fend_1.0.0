...
import { Banners, GymSection } from '../../../shared/interface/theme.interface';
...
export class GymComponent {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<GymSection>;
  @Select(CategoryState.category) category$: Observable<CategoryModel>;
  @Select(BrandState.brands) brand$: Observable<Select2Data>;

  public form: FormGroup;
  public page_data: GymSection;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePage({ slug: "gym" }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: GymSection) {
    const result: number[] = [];
