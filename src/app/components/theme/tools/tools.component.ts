...
import { Banners, Tools } from '../../../shared/interface/theme.interface';
...
export class ToolsComponent {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<Tools>;
  @Select(CategoryState.category) category$: Observable<CategoryModel>;
  @Select(BrandState.brands) brand$: Observable<Select2Data>;

  public form: FormGroup;
  public page_data: Tools;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePage({ slug: "tools" }));
  ...
  patchForm() {
    this.home_page$.subscribe(homePage => {
      ...
      this.selectedCategories2 = homePage?.content?.category_product?.right_panel?.product_category?.category_ids || [];
          ...
          category_product: {
            ...
            right_panel: {
              product_category: {
                ...
                category_ids: homePage?.content?.category_product?.right_panel?.product_category?.category_ids,
                status: homePage?.content?.category_product?.right_panel?.product_category?.status,
              },
              product_banner: {
                image_url: homePage?.content?.category_product?.right_panel?.product_banner?.image_url,
                status: homePage?.content?.category_product?.right_panel?.product_banner?.status,
                redirect_link: {
                  link: homePage?.content?.category_product?.right_panel?.product_banner?.redirect_link?.link,
                  link_type: homePage?.content?.category_product?.right_panel?.product_banner?.redirect_link?.link_type,
                  product_ids: homePage?.content?.category_product?.right_panel?.product_banner?.redirect_link?.product_ids
                },
              }
            }
          },
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: Tools) {
    const result: number[] = [];
