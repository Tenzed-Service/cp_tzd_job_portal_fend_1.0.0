...
import { Banners, VegetablesOne } from '../../../../shared/interface/theme.interface';
...
export class Vegetables1Component {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<VegetablesOne>;
  @Select(BrandState.brands) brand$: Observable<Select2Data>;

  public form: FormGroup;
  public page_data: VegetablesOne;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePage({ slug: "vegetables_one" }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: VegetablesOne) {
    const result: number[]= [];
