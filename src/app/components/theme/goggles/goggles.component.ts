...
import { Banners, Goggles } from '../../../shared/interface/theme.interface';
...
export class GogglesComponent {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<Goggles>;
  @Select(CategoryState.category) category$: Observable<CategoryModel>;
  @Select(BrandState.brands) brand$: Observable<Select2Data>;

  public form: FormGroup;
  public page_data: Goggles;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePage({ slug: "goggles" }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: Goggles) {
    const result: number[]= [];
