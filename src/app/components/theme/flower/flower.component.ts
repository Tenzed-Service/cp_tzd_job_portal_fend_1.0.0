...
import { Banners, Flower } from '../../../shared/interface/theme.interface';
...
export class FlowerComponent {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<Flower>;
  @Select(CategoryState.category) category$: Observable<CategoryModel>;
  @Select(BrandState.brands) brand$: Observable<Select2Data>;

  public form: FormGroup;
  public page_data: Flower;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePage({ slug: "flower" }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: Flower) {
    const result: number[] = [];
