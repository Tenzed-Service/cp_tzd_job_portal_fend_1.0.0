...
export class Marketplace1Component {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<MarketplaceOne>;
  ...
  patchForm() {
    this.home_page$.subscribe(homePage => {
      this.selectedCategories = homePage?.content?.category_product?.right_panel?.product_category?.category_ids || [];
