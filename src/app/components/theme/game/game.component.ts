...
import { Banners, Game } from '../../../shared/interface/theme.interface';
...
export class GameComponent {
  ...
  @Select(ThemeState.homePage) home_page$: Observable<Game>;
  @Select(CategoryState.category) category$: Observable<CategoryModel>;
  @Select(BrandState.brands) brand$: Observable<Select2Data>;

  public form: FormGroup;
  public page_data: Game;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePage({ slug: "game" }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: Game) {
    const result: number[]= [];
